...
Coupled with the military concerns, human errors in source code is yet another issue impeding roboticists to develop the software.
As computers scientists, fallible human beings, compile millions of lines of code into one perfect software, the risk of emergence of code that makes a robot to prone to fail in any given task is soaring.
A tiny flaw on software level, \citeA{lin} note, can lead to fatal results. 

Moreover, hacking has been the most debated issue since the Internet became publicly available.
Along with many other safety issues, hacking is an associated one regarding how to securely integrate robots in work settings.
In this case, the risk is not identity theft or any other illegal activities since the anticipations for robots to be employed in wide variety of tasks.
If a hacker can grant access in an uncanny way to robots' central processing unit, s/he can make the robot commit any action s/he wants.
So, all the superior abilities \citeA[p.~4]{lin}, namely, "its strength, ability to access and operate in difficult environments, expendability and so on", may be turned against society.
This possibility clearly contradicts the ultimate goal of creation of robots.
